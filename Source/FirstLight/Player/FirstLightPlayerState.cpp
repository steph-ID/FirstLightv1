...
AFirstLightPlayerState::AFirstLightPlayerState()
{
	...
	AttributeSetBase = CreateDefaultSubobject<UFLAttributeSetBase>(TEXT("AttributeSet"));
...
void AFirstLightPlayerState::OnRep_Level(int32 OldLevel)
{
	OnCharacterLevelChangedDelegate.Broadcast(Level);
}

void AFirstLightPlayerState::OnRep_ArenaLevel(int32 OldArenaLevel) const
{
	OnArenaLevelChangedDelegate.Broadcast(ArenaLevel);
}

void AFirstLightPlayerState::OnRep_ArenaXP(int32 OldArenaXP) const
{
	OnArenaXPChangedDelegate.Broadcast(ArenaXP);
}


void AFirstLightPlayerState::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	DOREPLIFETIME(AFirstLightPlayerState, Level);
	DOREPLIFETIME(AFirstLightPlayerState, ArenaLevel);
	DOREPLIFETIME(AFirstLightPlayerState, ArenaXP);
}
...
void AFirstLightPlayerState::AddToArenaXP(int32 IncomingArenaXP)
{
	ArenaXP += IncomingArenaXP;
	OnArenaXPChangedDelegate.Broadcast(ArenaXP);
}

void AFirstLightPlayerState::AddToArenaLevel(int32 IncomingArenaLevel)
{
	ArenaLevel += IncomingArenaLevel;
	OnArenaLevelChangedDelegate.Broadcast(ArenaLevel);
}

void AFirstLightPlayerState::SetArenaXP(int32 IncomingArenaXP)
{
	OnArenaXPChangedDelegate.Broadcast(ArenaXP);
}

void AFirstLightPlayerState::SetArenaLevel(int32 IncomingArenaLevel)
{
	OnArenaLevelChangedDelegate.Broadcast(ArenaLevel);
}
